...
exports.createBootcamp = asyncHandeler(async (req, res, next) => {
    ...
    req.body.user = req.user.id; // in mongoose id is getter of _id 

    // check for bublished bootcamp
    const publishedBootcamp = await Bootcamp.findOne({ user: req.user.id });

    // If the user is not an admin, the can only add one bootcamp
    if (publishedBootcamp && req.user.role !== "admin") {
        return next(new ErrorResponse(`The user with ID ${req.user.id} has already published a ${publishedBootcamp.name}`, 400));
    }
